...
class IcnNameConstructionServiceSpec extends Specification {
    ...
    static String setUpShardConfigs() {
        NameGroup.withTransaction { status ->
            [
                    [name: 'shard code' , value: 'ICN']
            ].each { data ->
                ShardConfig shardConfig = new ShardConfig(data)
                shardConfig.save()
            }
            ShardConfig.withSession { s -> s.flush() }
        }
        return "Added shard config records"
    }
    ...
    //constructName
    void "test name construction"() {
        when: "we construct a fullMarkedUpName and simpleMarkedUpName string"

        Author a1 = new Author(abbrev: 'A.S.George' , name: 'George, A.S.')
        Author a2 = new Author(abbrev: 'A.R.Mast &amp; K.R.Thiele' , name: 'Mast, A.R. & Thiele, K.R.')
        Name family = TestUte.makeName('family', 'Familia', null, testNameSpace).save()
        Name genus1 = TestUte.makeName('Banksia', 'Genus', family, testNameSpace).save()
        Name species1 = TestUte.makeName('acanthopoda', 'Species', genus1, testNameSpace).save()
        species1.author = a2
        species1.baseAuthor = a1
...
//        Name var2 = TestUte.makeName('var2', 'Varietas', species2, testNameSpace).save()
        ...
        println Name.list()
        Map output1 = service.constructName(species1)
        println(output1)
        Map output = service.constructName(species1)

        then:
        output['fullMarkedUpName'] == result_fullMarkedUpName
        and:
        output['simpleMarkedUpName'] == result_simpleMarkedUpName
        where:
        test              | result_fullMarkedUpName                                                                                                                                         |    result_simpleMarkedUpName
        "Ptilotus"        | '<scientific><name data-id=\'3\'><scientific><name data-id=\'2\'><element>Banksia</element></name></scientific> <element>acanthopoda</element> <authors>(<base data-id=\'null\' title=\'George, A.S.\'>A.S.George</base>) <author data-id=\'null\' title=\'Mast, A.R. &amp; Thiele, K.R.\'>A.R.Mast &amp; K.R.Thiele</author></authors></name></scientific>'      | '<scientific><name data-id=\'3\'><scientific><name data-id=\'2\'><element>Banksia</element></name></scientific> <element>acanthopoda</element></name></scientific>'
